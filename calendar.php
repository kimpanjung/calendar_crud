...
//				$gaingi_text = $lunarData[$i][1];
                ...
                $strTok =explode('.' , $lunarday);
                print_r($strTok[1]);

                $mul = (((int)$strTok[1]+6)%15);

                $gaingi_text = (((int)$strTok[1]+6)%15)."물";

            }
        }

        // 기념일 : 결혼, 생일, 제사, 기타 등
        if($memorialData[1] && count($memorialData[1]) >0){
        ...
        // 사용자 일정 데이터
		$dType1 = array();
      if($schedule[1] && count($schedule[1]) >0) {
